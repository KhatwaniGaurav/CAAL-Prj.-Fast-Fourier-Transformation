...
Reg_Clean:
    ...
    fadd.s fs6,fs6, ft3        # Load x into fs6 
    ...
    # --- Cosine Calculation ---
    la a0, one
    flw ft2, 0(a0)        # cos_result = 1.0 (final value kept)
...
cosine_loop:
    ...
    fmul.s ft3, ft3, ft0  # Update factorial
...
cosine_cleanup:
    # Clean cosine temporary registers
    fmv.s.x fs4, zero     # Clear fs4
    fmv.s.x ft0, zero     # Clear ft0
    fmv.s.x ft3, zero     # Clear ft3
    fmv.s.x ft5, zero     # Clear ft5
    li t0, 0              # Clear integer temps
    li t3, 0
    li t4, 0
    li t5, 0
    li t6, 0
    ...
    # --- Sine Calculation ---
    fmv.s ft1, fs6        # sin_result = x (final value kept)
...
sine_cleanup:
    # Clean sine temporary registers
    fmv.s.x fs0, zero     # Clear fs0
    fmv.s.x fs2, zero     # Clear fs2
    fmv.s.x fs3, zero     # Clear fs3
    fmv.s.x fs6, zero     # Clear fs6
    fmv.s.x ft0, zero     # Clear ft0
    fmv.s.x ft4, zero     # Clear ft4
    fmv.s.x ft5, zero     # Clear ft5
    li t0, 0              
    li t3, 0
    li t4, 0
    ...
    # ft2 ≈ cos(6.135923) ≈ -0.989 (correct) //Real Part
    # ft1 ≈ sin(6.135923) ≈ -0.149 (correct) //Imaginary Part